...
export default function ProductImagesPanel({ images, onAdd, onRemove, onSetFeatured, onReorder }: Props) {
    ...
    const reservedImages = images.slice(0,3);
    ...
    return (
        <>
            ...
            <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 8, overflow: "hidden" }}>
                <SectionHeader title="Reserved (Preview/Mask/Zoom)" count={gallery.length} />
                <div style={{ padding: 14 }}>
                    {reservedImages.length > 0 && (
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 12 }}>
                            {reservedImages.map((url, i) => (
                                <div
                                    key={url}
                                    draggable
